...
export const Mutation = {
  createTeam: async (
    ...
    try {
      ...
      const teams : TeamSchema[] = await TeamCollection.find({ players: { $in: players.map((p) => new ObjectId(p)) } }).toArray();
      if (teams.length > 0) {
  ...
  updateTeam: async (
    ...
    try {
      ...
      if (players) {
        ...
        const teams : TeamSchema[] = await TeamCollection.find({ players: { $in: players.map((p) => new ObjectId(p)) } }).toArray();
        if (teams.length > 0) {
            // no contar con el mismo equipo
            if (teams.length === 1 && teams[0]._id.toString() === id) {
                // no hacer nada
            }
            else {
                throw new Error("Player already has a team");
